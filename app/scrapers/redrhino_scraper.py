...
def scrape_website(url, limit=5, base_dir=None, headers=None):
    ...
    def get_next_page_url(soup, base_url):
        ...
        next_link = soup.find('a', class_='elementor-button-link', string=lambda x: x and 'Next Page' in x)
    ...
    def get_red_rhino_product_details(product_url):
        response = requests.get(product_url, headers=headers, verify=False)
        soup = BeautifulSoup(response.text, 'html.parser')

        product_name = soup.select_one('h1.product_title').text.strip()
    ...
    def process_red_rhino_product(product_name, product_url, image_url, product_soup, domain_dir, headers):
        try:
            session = Session()
            logger.info(f"Processing Red Rhino product: {product_name}")
            
            # Get metadata
            product_details = get_red_rhino_product_details(product_url)
    ...
    # Call the Red Rhino-specific functions
    ...
    while current_url and (limit == -1 or successful_downloads < limit):
        ...
        product_links = soup.select('a.elementor-button-link[href*="/firework/"]')
        ...
        for product_url in unique_product_links:
            ...
            try:
                ...
                if product_name:
                    ...
                    # Get product image
                    image_url = None
                    main_image = product_soup.select_one('img.attachment-large')
                    if main_image:
                    ...
                    if image_url:
                        ...
                        was_updated = process_red_rhino_product(product_name, product_url, image_url, product_soup, domain_dir, headers)
                        if was_updated:
